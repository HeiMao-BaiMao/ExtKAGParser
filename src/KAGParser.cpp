...
ttstr tTJSNI_KAGParser::GetTagName(const tjs_char ldelim)
{
	...
	if(IsEndOfLine() || GetCurChar() == ldelim)
		TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W(" ^ O     擾 ł  ܂   "), TVPCurPosErrorString);
	...
	if(tagnamestart == CurLineStr + CurPos)
		TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W(" ^ O     w 肳  Ă  ܂   "), TVPCurPosErrorString);
...
ttstr tTJSNI_KAGParser::GetAttributeName(const tjs_char ldelim)
{
	SkipWhiteSpace();
	if(IsEndOfLine() || GetCurChar() == ldelim)
		TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W(" ^ O ̑        擾 ł  ܂   "), TVPCurPosErrorString);
...
ttstr tTJSNI_KAGParser::GetAttributeValue(const tjs_char ldelim)
{
	...
	SkipWhiteSpace();	
	if(IsEndOfLine() || GetCurChar() == ldelim)
		TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W(" ^ O ̑    l   擾 ł  ܂   "), TVPCurPosErrorString);
		...
		(vdelim ? ch != vdelim : (ch != ldelim && !IsWhiteSpace(ch))))
	{
		if(ch == TJS_W('`'))
		{
			...
			CurPos++;
			if(IsEndOfLine())
				TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("'`'( o b N N H [ g)   オ s   ł "), TVPCurPosErrorString);
			ch = GetCurChar();
		}
		value += ch;
		CurPos++;
	...
	if((ldelim != 0 || (vdelim && GetCurChar() != vdelim)) && IsEndOfLine())
		TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("     l         A\'    \"  ̑Ή    Ԉ   Ă  ܂ "), TVPCurPosErrorString);
...
void tTJSNI_KAGParser::PopMacroArgs()
{
	if(MacroArgStackDepth == 0)
		TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W(" } N   X ^ b N    ̂  ߁APopMacroArg()      s ł  ܂   "), TVPCurPosErrorString);
	MacroArgStackDepth--;
}
//---------------------------------------------------------------------------
...
// TVPAddLog(TJS_W("CurLineStr = ") + ttstr(CurLineStr) + TJS_W(", CurPos = ") + ttstr(CurPos));
			...
			if(GetCurChar() == TJS_W('\\')) // check CurLineStr
			{
				...
				if(++CurLine >= LineCount)
					TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("     s ^ O( s   \\)   X N   v g     ɂ   ܂ "), TVPCurPosErrorString);
			...
			if(IsEndOfLine())
				TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W(" ^ O r   ōs   ɒB   ܂   "), TVPCurPosErrorString);
...
// ********************************************************************
		...
		if(tagkind == tag_if || tagkind == tag_ignore)
		{
			...
			if(NeedToDoThisTag())
			{
				...
				if(ttstr(val) == TJS_W(""))
					TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("if/ignore  ^ O   exp=    w 肳  Ă  ܂   "), TVPCurPosErrorString);
		...
		else if(tagkind == tag_elsif)
		{
			...
			else if(IfLevel == ExcludeLevel)
			{
				...
				if(exp == TJS_W(""))
					TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("elsif  ^ O   exp=    w 肳  Ă  ܂   "), TVPCurPosErrorString);
		...
		else if(tagkind == tag_while)
		{
			...
			if (NeedToDoThisTag())
			{
				...
				if (WhileLevelExp == TJS_W(""))
					TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("while  ^ O   exp=    w 肳  Ă  ܂   "), TVPCurPosErrorString);
...
// ********************************************************************
		...
		else if(tagkind == tag_emb || tagkind == tag_spemb)
		{
			...
			if(exp == TJS_W(""))
				TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("emb/&  ^ O   exp=    w 肳  Ă  ܂   "), TVPCurPosErrorString);
			SWITCH_TVPEXECUTEEXPRESSION(exp, Owner, &val);
			exp = val;
		...
		else if(tagkind == tag_macro)
		{
			...
			if(RecordingMacroName == TJS_W(""))
				TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("macro  ^ O   name=    w 肳  Ă  ܂   "), TVPCurPosErrorString);
					// missing macro name
		...
		else if(tagkind == tag_endmacro)
		{
			// end recording macro
			if(!RecordingMacro)
				TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("macro  ^ O Ȃ    endmacro  ^ O   g p    ܂   "), TVPCurPosErrorString);
			RecordingMacro = false;
			if(DebugLevel >= tkdlVerbose)
			{
		...
		else if(tagkind == tag_pmacro)
		{
			...
			if (pmacroname.Type() == tvtVoid)
				TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("pmacro  ^ O   name=       ܂   "), TVPCurPosErrorString);
		...
		{
			TVPThrowExceptionMessage(TVPKAGSyntaxError, TJS_W("   m ̃G   [ ł  tagkind=")+ttstr(tagkind), TVPCurPosErrorString);
		}
